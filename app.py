...
def leer_nombre_curso(nombre):
    try:
        cursor = conexion.connection.cursor()
        sql = "SELECT codigo, nombre, creditos FROM curso WHERE nombre = '{0}'".format(nombre)
        cursor.execute(sql)
        datos = cursor.fetchone()
        if datos != None:
            curso = {'codigo': datos[0], 'nombre': datos[1], 'creditos': datos[2]}
...
def registrar_curso():
    if (validar_codigo(request.json['codigo']) and validar_nombre(request.json['nombre']) and validar_creditos(request.json['creditos'])):
        try:
            ...
            nombre_curso = leer_nombre_curso(request.json['nombre'])
            if curso != None:
                return jsonify({'mensaje': "Código ya existe, no se puede duplicar."})
            elif nombre_curso != None:
                return jsonify({'mensaje': "Nombre de curso ya existe, no se puede duplicar."})
            else:                
                #print(request.json)
                cursor = conexion.connection.cursor()
